# Marketing Strategy - Week 1 Test

**Goal:** Test what works. Track engagement. Adjust after 1 week.

**Time budget:** 1+ hour/day
**Start date:** 2025-02-04 (Tuesday)
**Evaluation:** 2025-02-11 (Tuesday)

---

## Account Overview

| Account | Followers | Target | Vibe |
|---------|-----------|--------|------|
| Mulligan | ~6 | Casual golfers | Sarcastic roasts |
| Slothy | ~6 | ADHD / productivity | Relatable + funny |
| Dev | 600 (inactive) | Indie hackers | Behind the scenes |

---

## Posting Schedule (Swedish Time → International Reach)

All times are **Swedish time (CET)**. Optimized for USA + UK audiences.

| Swedish Time | USA (EST) | UK (GMT) | Best For |
|--------------|-----------|----------|----------|
| 15:00 | 9 AM | 2 PM | Reels (USA morning scroll) |
| 18:00 | 12 PM | 5 PM | Stories (USA lunch) |
| 21:00 | 3 PM | 8 PM | Posts (USA afternoon peak) |

### Weekly Schedule

| Day | Mulligan | Slothy | Dev |
|-----|----------|--------|-----|
| **Tue** | Post 21:00 | Post 21:30 | Post 22:00 |
| **Wed** | Story 18:00 | Story 18:30 | Story 18:00 |
| **Thu** | Reel 15:00 | Post 21:00 | Post 21:00 |
| **Fri** | Post 21:00 | Reel 15:00 | Story 18:00 |
| **Sat** | Story 18:00 | Story 18:00 | — |
| **Sun** | Post 21:00 | Post 21:00 | Post 21:00 |
| **Mon** | Reel 15:00 | Post 21:00 | Story 18:00 |

### Why These Times?
- **Posts 21:00:** Hits USA 3 PM = afternoon scrolling peak
- **Reels 15:00:** Hits USA 9 AM = morning scroll, reels perform well
- **Stories 18:00:** Hits USA lunch + UK evening
- **Best days:** Wednesday + Thursday get highest engagement globally

---

## Algorithm Secrets

### Most Important: "Sends per Reach"
Instagram now prioritizes how often people **DM your post to a friend**. More important than likes.

**How to get sends:**
- Relatable content ("omg this is so me")
- Funny/sarcastic takes people want to share
- Hot takes that make people tag friends

### First Hour is Critical
Engagement in the first 60 minutes determines if your post spreads.

**Action:** After posting, spend 15-20 min engaging on other posts in your niche. This drives traffic back to your fresh post.

---

## Daily Routine (1 hour)

| Time | Task |
|------|------|
| **15 min** | Create today's content |
| **5 min** | Post + hashtags |
| **20 min** | 5-3-1 engagement (see below) |
| **10 min** | Reply to any comments on your posts |
| **10 min** | Browse hashtags, engage more |

---

## 5-3-1 Rule (Do This Daily)

For each account, every day:

| Action | Amount | What to do |
|--------|--------|------------|
| **Like** | 5 posts | From your target hashtags |
| **Comment** | 3 posts | Meaningful/funny comments |
| **Follow** | 1 person | Your ideal follower |

**Total per day:** 15 likes, 9 comments, 3 follows across all accounts.

---

## Comment Strategy

### Your Style: Funny / Sarcastic

**DON'T write:**
- "Nice!"
- "Great post!"
- "🔥🔥🔥"
- "Love this!"

These are invisible to the algorithm and don't build connection.

### Comment Templates (Mulligan - Golf)

**On golf fail videos:**
> "This is why I keep a spare club in the car. And a spare car."

**On 'great shot' posts:**
> "Meanwhile my ball is still looking for its dad in the woods"

**On golf tips:**
> "Instructions unclear, hit the cart girl"

**On course photos:**
> "Beautiful. I'd find a way to lose a ball here too"

### Comment Templates (Slothy - ADHD/Productivity)

**On productivity tips:**
> "Saved this. Will never look at it again. As is tradition."

**On todo list posts:**
> "Bold of you to assume I remember where I put my list"

**On 'finally did the thing' posts:**
> "The executive function gods smiled upon you today"

**On ADHD memes:**
> "I feel personally attacked and I'm here for it"

### Comment Templates (Dev - Indie Hackers)

**On shipped products:**
> "Shipped > perfect. This is the way."

**On struggle posts:**
> "The grind is real. At least the bugs keep us company"

**On 'how I got X users':**
> "Taking notes. Probably won't follow them. But taking notes."

---

## Hashtag Strategy

### Mulligan (use 5-10 per post)
```
#golf #golfmemes #golflife #weekendgolfer #amateurgolf
#golfhumor #golfaddict #golfislife #golfproblems
```

**Niche hashtags (smaller, more targeted):**
```
#weekendgolf #casualgolfer #golffails #mulliganneeded
```

### Slothy (use 5-10 per post)
```
#adhd #adhdmemes #adhdlife #productivity #todolist
#adhdproblems #executivedysfunction #adhdawareness
```

**Niche hashtags:**
```
#adhdhumor #adhdstruggles #productivitystruggles #adhdcommunity
```

### Dev (use 5-10 per post)
```
#buildinpublic #indiehacker #solodev #startup #appdev
#shipit #makersmovement #indiedev
```

---

## Content Calendar - Week 1 (Swedish Time)

### Tuesday (Feb 4)

| Time (CET) | Account | Content |
|------------|---------|---------|
| 21:00 | Mulligan | **Mulligan Advice #1** — "Blame the wind. Always blame the wind. Even indoors." |
| 21:30 | Slothy | **ADHD meme** — "Made a todo list. Lost the todo list. Made a todo list to find the todo list." |
| 22:00 | Dev | **Intro post** — "Shipping 2 apps. Documenting everything. Follow for the chaos." |

### Wednesday (Feb 5)

| Time (CET) | Account | Content |
|------------|---------|---------|
| 18:00 | Mulligan | **Story** — Golf meme repost |
| 18:00 | Dev | **Story** — Screenshot of your roadmap |
| 18:30 | Slothy | **Story** — Productivity fail meme |

### Thursday (Feb 6)

| Time (CET) | Account | Content |
|------------|---------|---------|
| 15:00 | Mulligan | **Reel** — Mulligan Advice as text-on-screen video |
| 21:00 | Slothy | **Post** — "POV: You finally checked off one task and now you need a 3 hour break" |
| 21:00 | Dev | **Post** — "Got 110 testers in 12 days. Here's what actually worked:" (carousel or text post) |

### Friday (Feb 7)

| Time (CET) | Account | Content |
|------------|---------|---------|
| 15:00 | Slothy | **Reel** — Quick relatable ADHD moment |
| 18:00 | Dev | **Story** — Behind the scenes, your setup |
| 21:00 | Mulligan | **Post** — Golf meme / weekend golf anticipation |

### Saturday (Feb 8)

| Time (CET) | Account | Content |
|------------|---------|---------|
| 18:00 | Mulligan | **Story** — "Weekend golf mode activated" + app mention |
| 18:00 | Slothy | **Story** — "Weekend = no todo list guilt" |

### Sunday (Feb 9)

| Time (CET) | Account | Content |
|------------|---------|---------|
| 21:00 | Mulligan | **Post** — Mulligan Advice #2 |
| 21:00 | Slothy | **Post** — Monday dread meme (or schedule for Mon morning) |
| 21:00 | Dev | **Post** — Week reflection: what worked, what didn't |

### Monday (Feb 10)

| Time (CET) | Account | Content |
|------------|---------|---------|
| 15:00 | Mulligan | **Reel** — Best performing content repurposed OR new Mulligan Advice |
| 18:00 | Dev | **Story** — What you're working on this week |
| 21:00 | Slothy | **Post** — Fresh week energy (or anti-motivation meme) |

---

## Metrics to Track

Create a simple spreadsheet or note:

| Date | Account | Post Type | Reach | Likes | Comments | Saves | Shares | New Followers |
|------|---------|-----------|-------|-------|----------|-------|--------|---------------|

**Check daily:** Instagram Insights on each post

**Most important metrics:**
1. **Shares/Sends** — Algorithm loves this
2. **Saves** — Shows value
3. **Comments** — Engagement signal
4. **Reach** — How far it spread

---

## Week 1 Evaluation Questions (Feb 11)

Answer these after one week:

1. Which account got the most engagement?
2. Which content type performed best? (meme vs advice vs reel vs story)
3. Did any post get shared/saved more than others?
4. Which comments got replies? (your engagement working?)
5. Any follower growth?
6. What felt fun to make vs what felt like a chore?
7. What will you double down on week 2?

---

## Quick Reference

### Before Posting
- [ ] Content ready
- [ ] 5-10 hashtags prepared
- [ ] Caption written (include CTA if relevant)

### After Posting (first 20 min)
- [ ] Share to Stories
- [ ] Do 5-3-1 engagement
- [ ] Reply to any early comments

### Daily Non-Negotiables
- [ ] 1 post or story per active account
- [ ] 5-3-1 engagement per account
- [ ] Check and reply to comments

---

## Sources

- [Buffer: Best Time to Post on Instagram 2025](https://buffer.com/resources/when-is-the-best-time-to-post-on-instagram/)
- [Hootsuite: Instagram Algorithm 2026](https://blog.hootsuite.com/instagram-algorithm/)
- [Sprout Social: Best Times to Post](https://sproutsocial.com/insights/best-times-to-post-on-instagram/)
- [Influencer Marketing Hub: Instagram Growth Strategies](https://influencermarketinghub.com/instagram-growth-strategies/)

---

*Week 1 = testing. Don't stress results. Learn what works for YOU.*
